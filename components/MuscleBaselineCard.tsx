...
const MuscleBaselineCard: React.FC<MuscleBaselineCardProps> = ({
  ...
  return (
    <div className="rounded-lg border border-slate-300 dark:border-brand-muted bg-white dark:bg-brand-surface p-4 space-y-3">
      {/* Muscle Name Header */}
      <div className="flex items-center justify-between">
        <h4 className="text-lg font-bold text-slate-900 dark:text-slate-100">{muscleName}</h4>
      ...
      <div className="space-y-1">
        <p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">🤖 System Learned</p>
        <p className="text-sm text-slate-700 dark:text-slate-300">
          {baseline.systemLearnedMax.toLocaleString()} lbs
          {baseline.systemLearnedMax === 10000 && (
            <span className="ml-2 text-xs text-yellow-600 dark:text-yellow-400">(Default - No data yet)</span>
          )}
        </p>
      </div>
      ...
      <div className="space-y-2">
        <p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">✏️ Your Override</p>
        <div className="flex gap-2">
          <input
            ...
            min="100"
            max="1000000"
            className="flex-1 bg-slate-50 dark:bg-brand-dark border border-slate-300 dark:border-brand-muted rounded px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:border-brand-cyan focus:ring-1 focus:ring-brand-cyan outline-none"
          />
          {overrideInput && (
            <button
              onClick={handleClear}
              className="px-3 py-2 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 border border-red-300 dark:border-red-500/50 rounded hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors text-sm font-medium"
            >
              Clear
            </button>
          )}
      ...
      <div className="pt-3 border-t border-slate-300 dark:border-brand-muted space-y-1">
        <p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase">✅ Currently Using</p>
      ...
      {systemExceedsOverride && (
        <div className="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-500/50 rounded p-3 space-y-2">
          ...
          <button
            ...
            className="w-full py-2 px-4 bg-yellow-100 dark:bg-yellow-600/20 text-yellow-700 dark:text-yellow-300 border border-yellow-300 dark:border-yellow-500/50 rounded hover:bg-yellow-200 dark:hover:bg-yellow-600/30 transition-colors text-sm font-medium"
          >
