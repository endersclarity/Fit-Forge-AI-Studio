...
## Feature Overview
Create a dedicated Workout History page at `/workout/history` with card-grid layout, filtering, sorting, search, and delete functionality.

## User Requirements
- **Layout**: Cards/Grid view (Option B from mockups)
- **Details**: Inline expansion (accordion-style)
- **Filters**: Date range + Workout type + Exercise name search
- **Sorting**: Multiple options (recent, duration, volume)
- **Actions**: View + Delete + Search by exercise name
- **Empty State**: Educational content explaining workout tracking

## Implementation Tasks

### Task 1: Create WorkoutHistoryPage Component
**File:** `components/WorkoutHistoryPage.tsx`
...
**Verification:** Component compiles, displays workouts in card grid, filters/sorts work.
...
Add route (after other workout routes):
```typescript
<Route path="/workout/history" element={wrapPage(<WorkoutHistoryPage />)} />
```

**Verification:** Navigate to `/workout/history` and page loads.

---

### Task 3: Update WorkoutSaveModal to Navigate to History
**File:** `components/active-workout/WorkoutSaveModal.tsx`

No code changes needed - just update `ActiveWorkoutPage` handler.
...
**Verification:** After saving workout, clicking "View Workout History" navigates to `/workout/history`.

---

### Task 4: Add Delete Method to workoutsAPI
**File:** `api.ts`

Find the `workoutsAPI` export and add:
```typescript
export const workoutsAPI = {
  async getAll(): Promise<WorkoutResponse[]> {
    // existing code...
  },

  async create(workout: WorkoutSession): Promise<void> {
    // existing code...
  },
...
**Verification:** Delete button removes workout from database and UI.

---

### Task 5: Test Full Flow
**Test steps:**
1. Navigate to `/workout/history` directly
2. Verify empty state shows if no workouts
3. Add a workout via `/workout/builder` → `/workout/active`
4. Save workout and click "View Workout History"
5. Verify workout appears in grid
6. Test search filter (type "bench press")
7. Test type filter (select "Push")
8. Test date range filter
9. Test all sort options
10. Click workout card to expand inline
11. Click delete and confirm removal
12. Verify workout removed from list
...
## Summary
- Task 1: WorkoutHistoryPage component with cards/grid layout
- Task 2: Add route to App.tsx
- Task 3: Update modal to navigate to history page
- Task 4: Add delete API method
- Task 5: Full end-to-end testing
