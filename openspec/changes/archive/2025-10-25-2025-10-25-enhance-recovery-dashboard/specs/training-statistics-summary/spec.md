# Spec: Training Statistics Summary

**Capability:** `training-statistics-summary`
**Status:** Proposed
**Created:** 2025-10-25
...
**Component Location:** `components/QuickTrainingStats.tsx`
...
**Calculation Functions (in Dashboard.tsx):**
```typescript
...
```typescript
// Location: utils/statsHelpers.ts (NEW FILE)
function calculateStreak(workouts: WorkoutSession[]): number;
function calculateWeeklyStats(workouts: WorkoutSession[]): { thisWeek: number; lastWeek: number };
function findRecentPRs(personalBests: PersonalBestsResponse, workouts: WorkoutSession[]): PRHighlight[];
...
**Dependencies:**
- `types.ts` (WorkoutSession, PersonalBestsResponse)
- `utils/helpers.ts` (date utilities if needed)
- React.memo for performance

**Styling:**
- Grid layout with Tailwind responsive utilities
- Consistent card design matching existing Dashboard sections
- Trend arrows: ↑ (green), ↓ (red), → (gray)

---

*This specification defines the training statistics summary that motivates users and provides quick awareness of their training progress.*
