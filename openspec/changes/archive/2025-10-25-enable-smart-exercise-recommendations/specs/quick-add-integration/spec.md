# Spec: Quick-Add Integration

**Capability:** `quick-add-integration`
**Status:** Proposed
**Created:** 2025-10-25
...
### Requirement: "Add to Workout" Button on Recommendation Cards
...
### Requirement: Navigate to Workout Screen with Pre-Selected Exercise
...
### Requirement: Integrate with Progressive Overload System
...
### Requirement: Handle Multiple Exercise Additions
...
### Requirement: Visual Feedback on Add Action
...
### Requirement: Prevent Duplicate Exercise Additions
...
### Requirement: Update Workout Context
...
### Requirement: Category Auto-Selection
...
### Requirement: Mobile Gesture Support
...
### Requirement: Integration with Existing Workout Flow
...
function handleAddExerciseToWorkout(exercise: Exercise) {
  // If no workout in progress, create new workout with exercise's category
  if (!currentWorkout) {
    const newWorkout = createWorkoutSession(exercise.category);
    setCurrentWorkout(newWorkout);
  }

  // Add exercise to current workout
  addExerciseToSession(currentWorkout, exercise);

  // Apply progressive overload if available
  const lastPerformance = getLastPerformance(exercise.name, exercise.category);
  if (lastPerformance) {
...
<ExerciseRecommendations
  ...
  currentWorkout={currentWorkout}  // Check for duplicates
/>

// ExerciseRecommendations.tsx
<RecommendationCard
  recommendation={rec}
  onAdd={() => onAddToWorkout(rec.exercise)}
  isInWorkout={isExerciseInCurrentWorkout(rec.exercise.name)}
/>
```

**Navigation:**
- Use React Router or state-based navigation
- Pass workout context via global state or context API
- Ensure workout state persists during navigation

---

*These requirements define a seamless integration between exercise recommendations and workout logging, minimizing user friction and maximizing training efficiency.*
