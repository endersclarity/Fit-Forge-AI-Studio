# Capability: Target-Driven Workout Generation

## Overview

Enable users to set desired muscle fatigue targets and automatically generate exercise recommendations with optimal volume to achieve those targets.

**Related Capabilities:**
- `volume-slider-ui` - Generated exercises use volume calculations
- `exercise-history-api` - Recommendations use historical performance data
- `exercise-recommendation-algorithm` - Extends existing recommendation logic
...
_None - This extends but doesn't modify existing capabilities_
...
**Greedy Approach:**
1. Sort targets by fatigue gap (largest first)
2. For each target:
   - Score all exercises
   - Pick highest scoring exercise
   - Calculate volume to close gap
   - Check constraints (skip if violation)
   - Update simulated muscle states
3. Return recommendations

**Complexity:** O(n × m) where n=targets, m=exercises (< 500ms for 3×48)
