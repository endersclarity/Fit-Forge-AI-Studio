...
This spec defines the frontend user interface components for frictionless exercise logging. The UI provides a 3-step flow (Exercise Selection → Weight/Reps Input → Submit) with smart defaults, progressive overload suggestions, and real-time feedback.

**Related Capabilities:**
- `quick-add-api` - Backend API consumed by these components
- `quick-add-history-integration` - History display for logged quick-adds

**Related Specs:**
- `progressive-overload-suggestions` - Uses progressive overload formulas
- `muscle-heat-map-visualization` - Integrates with Dashboard muscle states
- `recommendation-ui-display` - Shares ExercisePicker component
...
<QuickAddForm>
  ...
  <ProgressiveSuggestion defaults={smartDefaults} />
  <WeightInput value={weight} onChange={setWeight} />
  <RepsInput value={reps} onChange={setReps} />
  <ToFailureCheckbox checked={toFailure} onChange={setToFailure} />
  <ButtonGroup>
    <PrimaryButton onClick={onSubmit}>Log It</PrimaryButton>
    <SecondaryButton onClick={onBack}>Change Exercise</SecondaryButton>
  </ButtonGroup>
</QuickAddForm>
```
...
- **Position:** Fixed, bottom-right, 24px offset (desktop), 16px (mobile)
- **Background:** Linear gradient #667eea → #764ba2
- **Icon:** Lightning bolt (⚡) or plus (+)
- **Shadow:** 0 4px 12px rgba(0,0,0,0.15)
- **Hover:** Scale 1.05, shadow 0 6px 16px rgba(0,0,0,0.2)
- **Active:** Scale 0.95

### Modal Styling
- **Width:** 600px (desktop), 100% - 32px margins (mobile)
- **Max-height:** 90vh
- **Background:** White with border-radius 12px
...
- **Border:** 2px solid #e2e8f0, focus: #667eea
- **Border-radius:** 8px
- **Increment buttons:** 36px × 36px, positioned inline
...
|-----------|--------|-------------|
| FAB click → Modal open | < 100ms | Time from click to visible modal |
| Exercise selection → Form display | < 50ms | State update + re-render |
| Smart defaults fetch | < 100ms | API call + data processing |
| Input value change | < 16ms | Immediate visual feedback (60fps) |
| Form submit → Toast | < 300ms | Total flow time |
...
### Component Tests
- [ ] FAB renders and opens modal
- [ ] ExercisePicker filters by search term
- [ ] ExercisePicker filters by category
- [ ] Recent exercises display correctly
- [ ] Smart defaults pre-fill correctly
- [ ] Weight increment/decrement buttons work
- [ ] Reps increment/decrement buttons work
- [ ] To failure checkbox toggles
- [ ] Form validation prevents invalid submission
- [ ] Loading state during submission
- [ ] Success toast shows with correct message
- [ ] PR toast shows when applicable
- [ ] Modal closes after success
- [ ] Change exercise navigation works
- [ ] Keyboard navigation functions
- [ ] Mobile responsive layout renders

### Integration Tests
- [ ] Quick-add triggers muscle state refresh
- [ ] PR notification appears correctly
- [ ] History updates with quick-add
- [ ] Smart defaults fetch and display
- [ ] Progressive overload suggestion accurate
...
**Requires:**
- `components/Modal.tsx` - Existing modal component
- `components/Toast.tsx` - Existing toast notifications
- `components/ExerciseCard.tsx` - Existing exercise display
- `utils/smartDefaults.ts` - New smart defaults service
- `api.ts` - API client with quick-add endpoint

**Provides:**
- `components/QuickAdd.tsx` - Main modal component
- `components/QuickAddForm.tsx` - Input form
- `components/ExercisePicker.tsx` - Exercise selection (enhanced or new)
- `components/ProgressiveSuggestion.tsx` - Overload suggestion card

---

*This spec ensures the quick-add UI provides a frictionless, accessible, and delightful experience for logging exercises while integrating seamlessly with Dashboard muscle states and recommendations.*
