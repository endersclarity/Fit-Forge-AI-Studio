...
- **Related Capabilities:**
  - `quick-start-recommendation` (modified in this change)
  - `workout-history-display` (modified in this change)
  - `muscle-visualization-hover-detection` (existing, unchanged)

- **Related Changes:**
  - Future: `implement-workout-rotation-logic` (will enhance Quick Start logic)
  - Future: `improve-navigation-icon-clarity` (separate UX cleanup)

---

## Notes

This spec addresses user feedback from USER_FEEDBACK.md (2025-10-27):
- "Workout history appears THREE times" → Now appears once, collapsed
- "Welcome back tester, ready to forge strength" → Simplified to name only
- "Homepage should lead with large muscular structure" → Muscle viz is hero element
- "Don't need workout recommendations up front" → Progressive disclosure
