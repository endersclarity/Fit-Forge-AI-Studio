...
**Description:** System SHALL apply sensible default failure status.
...
**Description:** System SHALL update API request schema to include `to_failure` field.
...
- Add index: `CREATE INDEX idx_exercise_sets_to_failure ON exercise_sets(to_failure)`
...
*This spec ensures failure status tracking is simple, intuitive, and lays the foundation for future intelligent features while providing immediate value to users.*
