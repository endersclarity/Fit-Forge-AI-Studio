# Spec: Recovery Dashboard Screen

**Capability:** `recovery-dashboard-screen`
**Status:** Draft
**Change:** `implement-recovery-dashboard-components`

---

## Overview

The Recovery Dashboard is the primary home screen of FitForge, providing users with instant muscle recovery intelligence and smart exercise recommendations. Upon opening the app, users immediately see a visual heat map of 13 muscle groups with color-coded fatigue levels, along with personalized workout suggestions based on current recovery states.

**Core Behavior:** When a user opens the app, the dashboard loads muscle states from the backend API, displays them in a collapsible heat map organized by category (Push/Pull/Legs/Core), and presents smart exercise recommendations with status badges indicating workout quality.
...
**Previous Behavior:** App has placeholder home screen or no home screen

**New Behavior:** Recovery Dashboard is the primary home screen, replacing placeholder UI

**Changes:**
- App routing updated: `/` → RecoveryDashboard
- Bottom navigation "Dashboard" item active by default
- Deep links to dashboard work correctly
- Browser back button returns to dashboard when appropriate

#### Scenario: User opens app

**Given:** User has closed the app
**When:** User taps app icon
**Then:** Recovery Dashboard loads as home screen
**And:** Bottom navigation shows "Dashboard" as active
**And:** URL is `/dashboard` or `/`

#### Scenario: User navigates away and returns

**Given:** User is on Workout History screen
**When:** User taps "Dashboard" in bottom nav
**Then:** Dashboard loads
**And:** Dashboard state is preserved (scroll position, expanded categories)
**Or:** Dashboard refreshes data if >1 minute stale

---

### Modified: Workout Flow

**Previous Behavior:** Users select workout template manually without context

**New Behavior:** Users are guided by muscle recovery intelligence via dashboard

**Changes:**
- FAB button "Start Workout" prominent on dashboard
- Tapping FAB navigates to workout selection with recommended category pre-selected
- Recommended workout type passed as route param
- Muscle states inform workout selection

#### Scenario: User starts workout from dashboard

**Given:** Dashboard shows "Ready for: PULL DAY B"
**When:** User taps FAB (Start Workout)
**Then:** Navigates to workout screen
**And:** PULL category pre-selected
**And:** Variation B highlighted as recommended
**And:** User can override selection if desired
...
**Scenario:** Brand new user with no workout data
**Handling:**
- Hero shows: "Welcome! Complete your first workout to start tracking recovery."
- Muscle heat map shows all muscles at 0% fatigue (green)
- Recommendations show: "Build your baseline - try any exercises to get started"
- Onboarding tips displayed

### Edge Case: Stale Data (>7 days)

**Scenario:** User hasn't logged workout in over 7 days
**Handling:**
- All muscles show 0% fatigue (fully recovered)
- Hero shows: "All muscles recovered. Time to get back to training!"
- Last workout shows: "Last workout: 8 days ago"
- Gentle encouragement message

### Edge Case: Conflicting Muscle States

**Scenario:** Database returns invalid fatigue percentages (<0 or >100)
**Handling:**
- Clamp values: `Math.max(0, Math.min(100, fatiguePercent))`
- Log error to monitoring system
- Display clamped value to user
- No crash, graceful degradation

### Edge Case: Missing Muscle Data

**Scenario:** API returns only 10 muscles instead of 13
**Handling:**
- Display available muscles
- Fill missing muscles with default state (0% fatigue, "Never trained")
- Log warning to monitoring
- Show partial data rather than failing

### Edge Case: Extremely Long Exercise Names

**Scenario:** Exercise name exceeds expected length
**Handling:**
- Truncate with ellipsis: "Bulgarian Split Squat wi..."
- Full name in tooltip on hover
- Accessible full name via screen reader
...
|--------|--------|-------------------|
| Initial Load (Skeleton) | < 500ms | Performance.now() |
| API Response (Muscle States) | < 800ms | Network tab |
| API Response (Recommendations) | < 1000ms | Network tab |
| Total Dashboard Load | < 2 seconds | Lighthouse |
| Collapsible Animation | 500ms smooth | 60fps, DevTools Performance |
| Scroll Performance | 60fps | Chrome DevTools FPS meter |
| Memory Usage | < 50MB | Chrome Task Manager |
| Bundle Size (Dashboard) | < 200KB gzipped | webpack-bundle-analyzer |
...
| Safari | 14+ | Webkit quirks tested |
| Edge | 90+ (Chromium) | Same as Chrome |
| Mobile Safari (iOS) | 14+ | Touch interactions |
| Chrome Mobile (Android) | 90+ | Touch interactions |

### Unsupported

- Internet Explorer (all versions)
- Safari < 14 (lacks CSS features)

### Graceful Degradation

- CSS Grid fallback: Flexbox layout
- CSS custom properties fallback: Static colors
- Modern JS features: Polyfills via Babel
...
- CSRF tokens validated on mutation requests
- Rate limiting: 60 requests/minute per user
- Data sanitized on backend (prevent XSS)

### Frontend Security

- No sensitive data stored in localStorage (only cache)
- API keys/secrets in environment variables (never client-side)
...
- Performance issues tracked (slow API, janky animations)

### User Analytics

- Dashboard load time (percentile distribution)
- Most viewed muscle categories
- Most tapped recommendations
- FAB tap rate (conversion to workout)
...
- [ ] All 13 muscles display with accurate fatigue percentages
- [ ] Color coding matches spec (green 0-33%, amber 34-66%, red 67-100%)
- [ ] Categories collapse/expand smoothly (500ms animation)
- [ ] Recommendations load and display status badges correctly
- [ ] Progressive overload chips show +3% calculations
- [ ] Keyboard navigation works (all elements tabbable)
- [ ] Screen reader announces all content correctly
- [ ] WCAG AAA compliance verified (WAVE: 0 errors, axe: 0 critical)
- [ ] Dashboard loads in <1 second on 4G
- [ ] Offline mode works (shows cached data)
- [ ] Skeleton screens prevent layout shift
- [ ] Error states display user-friendly messages
- [ ] Respects `prefers-reduced-motion` setting
- [ ] Works on mobile (iPhone SE 375px, Pixel 5 393px)
...
*This specification defines the complete behavior, edge cases, and acceptance criteria for the Recovery Dashboard screen capability.*
